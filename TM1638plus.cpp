...
void TM1638plus::display7Seg(uint8_t position, uint8_t value) { // call 7-segment
  sendCommand(WRITE_LOC);
  digitalWrite(_STROBE_IO, LOW);
  shiftOutMod(_DATA_IO, _CLOCK_IO, LSBFIRST, SEG_ADR + (position << 1));
  shiftOutMod(_DATA_IO, _CLOCK_IO, LSBFIRST, value);
  digitalWrite(_STROBE_IO, HIGH); 
}
...
uint8_t TM1638plus::readButtons()
{
  uint8_t buttons = 0;
  digitalWrite(_STROBE_IO, LOW);
  shiftOutMod(_DATA_IO, _CLOCK_IO, LSBFIRST, BUTTONS_MODE);
  pinMode(_DATA_IO, INPUT);
  

  for (uint8_t i = 0; i < 4; i++)
  {
    uint8_t v = shiftInMod(_DATA_IO, _CLOCK_IO, LSBFIRST) << i;
    buttons |= v;
  }
